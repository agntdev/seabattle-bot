# fix-b339d96c74f3764f — profile-store ensure() is a no-op, never persists new profiles

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SBOT

`ProfileStore.ensure(userId)` calls `this.get(userId)` and returns the result. For a new user, `get()` returns the `DEFAULT_PROFILE` without persisting it. The method is named `ensure` (implying "ensure the profile exists in storage") but never calls `set()`, so the profile is never written to Redis/memory. Callers expecting `ensure()` to create a persistent profile entry will get a transient default that is lost across restarts until an explicit `set()` call is made.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-b339d96c74f3764f.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-b339d96c74f3764f.json` (and any new command `tests/commands/fix-b339d96c74f3764f.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-b339d96c74f3764f.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-b339d96c74f3764f.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
