# fix-ffb374bbbc2c8eae — E7T2 /turnt passes the turn without verifying a move was made, enabling turn-skipping abuse

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SBOT

`/turnt` calls `matchStorage.passTurn()` (line 37) without checking whether the current player has actually recorded a move. A player whose turn it currently is can call `/turnt` to immediately pass the turn back, giving the opponent consecutive turns while producing a misleading notification ("Your opponent has made a move"). The `/turnt` command should either (a) verify a move was made before passing, or (b) function purely as a notification relay without altering turn state.

**Test-confirmed:** The E7T2 "full flow" test shows player B (88) calling `/turnt 1` after player A fired, which passes turn BACK to player A without player B having fired. Player A can then fire again.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-ffb374bbbc2c8eae.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-ffb374bbbc2c8eae.json` (and any new command `tests/commands/fix-ffb374bbbc2c8eae.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-ffb374bbbc2c8eae.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-ffb374bbbc2c8eae.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
