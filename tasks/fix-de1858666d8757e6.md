# fix-de1858666d8757e6 — E4T2: zrem sends multiple members incorrectly in mm:wait

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SBOT

Line 92: `await redis.zrem(QUEUE_ZSET, userId.toString(), opponentId)` — ioredis `zrem` accepts `...members` variadic args. With the fake `DomainRedis` interface from `persistent.ts` used via `getRedisClient()`, the cast `as unknown as RatingQueueRedis` provides `zadd/zrem/zrangebyscore` but `persistent.ts`'s real Redis client doesn't implement these commands in its `DomainRedis` interface (it only exposes `set/get/del/sadd/lpush`). The runtime ioredis client supports `zrem` natively, so this works at runtime, but it bypasses the type-intended interface and reveals a broken abstraction.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-de1858666d8757e6.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-de1858666d8757e6.json` (and any new command `tests/commands/fix-de1858666d8757e6.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-de1858666d8757e6.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-de1858666d8757e6.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
