# fix-a80461b07288c90d — E1T3: `answerCallbackQuery` fires after message reply, keeping spinner alive

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SBOT

`ctx.answerCallbackQuery()` is called on line 39, AFTER `editMessageText`/`reply`. This keeps the Telegram inline-button loading spinner visible until profile retrieval and rendering complete. E1T1 and E1T2 answer the callback FIRST, dismissing the spinner before any potentially slow I/O. Moving `answerCallbackQuery` to before the profile fetch (line 9) would match the other handlers' pattern.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-a80461b07288c90d.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-a80461b07288c90d.json` (and any new command `tests/commands/fix-a80461b07288c90d.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-a80461b07288c90d.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-a80461b07288c90d.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
