# fix-9c13d05c12465980 — E7T1 seedOpponentBoard silently ignores non-duplicate placement errors

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SBOT

`seedOpponentBoard()` (line 55-68) iterates over `FIXED_PLACEMENTS` calling `boardStorage.placeShip()`. It only handles `error === 'duplicate'` (line 64). Errors like `'bounds'` or `'overlap'` are silently ignored — `continue` is never reached, and the placement silently fails. If the opponent's board already has ships from a different placement, the overlapping placement attempt fails silently, and the opponent board is incomplete with no error surfaced to the attacker.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-9c13d05c12465980.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-9c13d05c12465980.json` (and any new command `tests/commands/fix-9c13d05c12465980.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-9c13d05c12465980.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-9c13d05c12465980.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
