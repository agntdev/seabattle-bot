# fix-d81ee8ceee42416d — E9T5 handler: /checkinvite truncates multi-word code input

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SBOT

In `E9T5.ts`, the `/checkinvite` handler extracts the code as `const code = args[1]`, which only captures the first whitespace-delimited token. When the user sends `/checkinvite bad code!`, the handler gets `code="bad"` (all letters, passes `validateCode`), then looks up `"bad"` in storage, finds nothing, and replies `"Invite not found."`. The `invite-model.json` test for this exact input **expects** `"Invalid code: Code may only contain letters, digits, hyphens, and underscores."` — a validation error for the full string `"bad code!"`. The handler should join remaining args (e.g. `args.slice(1).join(" ")`) so the full input is validated. This test will fail.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-d81ee8ceee42416d.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-d81ee8ceee42416d.json` (and any new command `tests/commands/fix-d81ee8ceee42416d.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-d81ee8ceee42416d.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-d81ee8ceee42416d.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
