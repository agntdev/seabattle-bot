# fix-cccc57dae997efd8 — E7T1: hardcoded opponent derivation — not a real match system

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SBOT

`/attack` derives `opponentId` as `ctx.chat.id + 1` (line 96). This is fake/hardcoded data that does not correspond to any actual match, opponent, or game state. It assumes the opponent is always `chatId+1`, which is a test stub, not a real implementation. Per AGENTS.md, stubs are a failure — this makes the entire attack flow non-functional in production.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-cccc57dae997efd8.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-cccc57dae997efd8.json` (and any new command `tests/commands/fix-cccc57dae997efd8.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-cccc57dae997efd8.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-cccc57dae997efd8.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
