# fix-4ee608ddeb45e3dd — Race condition: non-atomic Redis counter increments

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SBOT

Three model files use a read-modify-write pattern for auto-incrementing IDs: `match.ts:nextId()` (line 44-50), `move.ts:nextId()` (line 50-56), and `invite.ts:nextCode()` (line 43-49). Each does: `GET counter_key` → `parseInt` → increment → `SET counter_key`. Under concurrent access (e.g. two simultaneous match creates), both callers can read the same counter value and write back the same incremented value, producing **duplicate IDs**. The `RedisLike` interface in the toolkit (`session/redis.ts:18-23`) does **not expose `incr`**, so a fix would require either extending `RedisLike` or using a Lua script via `eval`. The Memory-based storage variants are not affected (single-threaded JS).

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-4ee608ddeb45e3dd.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-4ee608ddeb45e3dd.json` (and any new command `tests/commands/fix-4ee608ddeb45e3dd.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-4ee608ddeb45e3dd.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-4ee608ddeb45e3dd.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
