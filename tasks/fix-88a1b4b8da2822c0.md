# fix-88a1b4b8da2822c0 — E9T4: /fire and /history select wrong match with multiple active games

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 SBOT

Both `/fire` and `/history` pick `active[0]` from `matchStorage.findByPlayer()` with no user-facing way to specify which of several active matches to operate on. If a player has 2+ active matches, `/fire` targets the first match returned by storage (non-deterministic with Redis `keys()`), potentially firing at the wrong opponent's board. Similarly `/history` shows moves from an arbitrary match.

## Dialog tests

This is a FIX task: the behavior it repairs is already covered by an existing spec under `tests/specs/`. Fix the code to make that existing spec pass — do NOT author a new `tests/specs/fix-88a1b4b8da2822c0.json` (a duplicate spec for the same behavior makes the tests-gate count it twice and it can never go green). Add a new spec file ONLY if you are introducing genuinely new user-facing behavior that no existing spec covers; if so, name it `tests/specs/fix-88a1b4b8da2822c0.json` (and any new command `tests/commands/fix-88a1b4b8da2822c0.json`).


## Handler module

This is a FIX task. Find the EXISTING handler under `src/handlers/` that implements the affected command/behavior and EDIT it in place. Do NOT create a new `src/handlers/fix-88a1b4b8da2822c0.ts` — a second `Composer` binding the same command conflicts with the original and breaks the bot. Create a new handler file ONLY if the affected command does not exist anywhere yet (then name it `src/handlers/fix-88a1b4b8da2822c0.ts` and default-export a grammY `Composer`; `buildBot()` auto-loads it). NEVER edit `src/bot.ts`; the global error boundary + unknown-command fallback already live in `buildBot()`.
